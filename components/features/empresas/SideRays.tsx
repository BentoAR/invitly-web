...
export function SideRays({
  ...
  useEffect(() => {
    ...
    // Capturamos referencias locales para el cleanup, evitando acceder
    // a refs durante la fase de render (regla react-hooks/refs).
    const cleanupProgram = programRef.current;
    const cleanupGeometry = geometryRef.current;
    const cleanupGl = renderer.gl;
    const cleanupCanvas = renderer.gl.canvas;
    ...
    return () => {
      ...
      try {
        cleanupProgram?.remove();
        cleanupGeometry?.remove();
        const loseCtx = cleanupGl.getExtension("WEBGL_lose_context");
        if (loseCtx) loseCtx.loseContext();
        if (cleanupCanvas.parentNode) {
          cleanupCanvas.parentNode.removeChild(cleanupCanvas);
        }
      } catch {
        // ignore
      }
    ...
    // El init solo depende de isVisible; los cambios de props actualizan uniforms via otro effect.
